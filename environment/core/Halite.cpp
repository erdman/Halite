...
Halite::Halite(unsigned short width_, unsigned short height_, unsigned int seed_, unsigned short n_players_for_map_creation, Networking networking_, bool shouldIgnoreTimeout) {
    ...
    //Initialize map
    game_map = hlt::Map(width_, height_, std::max(number_of_players, n_players_for_map_creation), seed_);

    //If n_players_for_map_creation > 1, this is single-player mode, so remove all the extra players (they were automatically inserted in map, just 0 them out)
    if (n_players_for_map_creation > 1){
